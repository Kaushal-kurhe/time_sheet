...
function initClock() {
  ...
  document.getElementById("date-display").textContent = d.toLocaleDateString(
    "en-IN",
...
function stopTimer() {
  ...
  if (d > 0) {
    entries.unshift({
      id: Date.now(),
      company: timerMeta.company,
      project: timerMeta.project,
      date: timerMeta.date,
      start: timerMeta.start,
      end: endTime,
      duration: d,
      notes: "(timer)",
    });
  ...
  tb.innerHTML = list
    .map(
      ...
      <tr>
        ...
        <td><button class="del-btn" onclick="deleteEntry(${e.id})" aria-label="Delete entry"><i class="ti ti-trash"></i></button></td>
      </tr>`,
    )
    .join("");
}
...
function renderCompanies() {
  ...
  if (period === "today") filtered = entries.filter((e) => e.date === today());
    ...
    filtered = entries.filter((e) => e.date >= weekStart());
  else if (period === "month")
    filtered = entries.filter((e) => e.date >= monthStart());
  ...
  if (!sorted.length) {
    chart.innerHTML =
      '<div class="empty"><i class="ti ti-chart-bar"></i>No data for this period</div>';
  ...
  document.getElementById("company-tbody").innerHTML = sorted
    .map(
      ([name, data], i) => `
      <tr>
...
function renderCompanyModal() {
  document.getElementById("company-list-modal").innerHTML =
    companies
      ...
      .join("") ||
    '<div style="font-size:13px;color:var(--text-secondary);padding:8px 0">No companies yet.</div>';
}
